...
def dispatch(part_handlers, filename, payload, headers):
    ...
    try:
        if handler.handler_version == 3:
            handler.handle_part(
                None, ctype, filename, payload, PER_INSTANCE, headers
            )
        else:
...
def main() -> int:
    ...
    jinja = JinjaTemplatePartHandler(
        paths, sub_handlers=[script, boothook, cloud_config] + by_freq
    )

    part_handlers = {}
    all_handlers = [script, boothook, jinja, cloud_config] + by_freq
    for handler in all_handlers:
        for ctype in handler.list_types():
            part_handlers[ctype] = handler

    processed = user_data.UserDataProcessor(paths).process(blob)
    # Once per handler, not once per content type: a second CONTENT_END would
    # rewrite the merged config from the buffer the first one just reset.
    for handler in all_handlers:
        handlers.call_begin(handler, None, PER_INSTANCE)
    for part in processed.walk():
        ...
        dispatch(part_handlers, filename, payload, headers)
    for handler in all_handlers:
        handlers.call_end(handler, None, PER_INSTANCE)

    # `dump-handlers` prints with `serde_json::to_string_pretty` -- see the
    # note in userdata.py.
