//! Print what `read_metadata_service` makes of an `OpenStack` metadata service.

fn main() {
    let Some(base) = std::env::args().nth(1) else {
        eprintln!("usage: dump-openstack URL");
        std::process::exit(2);
    };
    ...
    match ci_datasource::helpers::openstack::read_metadata_service(&base, &config) {
