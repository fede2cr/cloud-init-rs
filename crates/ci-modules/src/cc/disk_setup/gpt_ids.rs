/// `sgdisk_to_gpt_id`: gdisk's own two-byte type codes, in the order
/// upstream lists them. Generated from the packaged source, which was
/// itself generated from `parttypes.cc` by the recipe in its comment.
pub const SGDISK_TO_GPT_ID: [(&str, &str); 362] = [
