...
/// `distros.uses_systemd`: whether `/run/systemd/system` is a directory.
///
/// `lstat`, not `stat`: upstream deliberately does not follow a symlink here,
/// so a `/run/systemd/system` that points at a directory still answers no.
#[must_use]
pub fn uses_systemd() -> bool {
    std::fs::symlink_metadata("/run/systemd/system").is_ok_and(|meta| meta.is_dir())
}
