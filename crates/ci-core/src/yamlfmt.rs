...
/// `Emitter.best_indent`, from `safeyaml.dumps(indent=4)`. Plain `yaml.dump`
/// leaves it at `PyYAML`'s own default of 2.
...
/// `safeyaml.dumps(explicit_start=False, explicit_end=False)`, as the netplan
/// renderer calls it. Same layout, without the document markers.
#[must_use]
pub fn dumps_bare(value: &Value) -> String {
    dumps_indented(value, BEST_INDENT)
}

/// Plain `yaml.dump(value, default_flow_style=False)`, which is `dumps_bare`
/// at `PyYAML`'s default indent of 2 rather than cloud-init's 4.
#[must_use]
pub fn dumps_plain(value: &Value) -> String {
    dumps_indented(value, 2)
}

fn dumps_indented(value: &Value, best_indent: usize) -> String {
    let mut emitter = Emitter {
        out: String::new(),
        column: 0,
        indent: None,
...
#[expect(
    ...
    reason = "these are PyYAML's Emitter attributes, one for one"
)]
struct Emitter {
    out: String,
    column: usize,
    ...
    whitespace: bool,
    indention: bool,
    /// `Emitter.root_context`: true until the first collection is entered, so
    /// a document that is one plain scalar can be told apart from one that is
    /// not.
    root: bool,
    /// `Emitter.open_ended`, set by `write_plain` in the root context. A
    /// document that ends in a plain scalar needs an explicit `...` to close
    /// it, whether or not `explicit_end` was asked for.
    open_ended: bool,
}
...
impl Emitter {
    ...
    fn increase_indent(&mut self, flow: bool, indentless: bool) -> Option<usize> {
        ...
        match self.indent {
            ...
            Some(current) if !indentless => {
                self.indent = Some(current + self.best_indent);
            }
            Some(_) => {}
        }
        saved
    ...
    fn emit_node(&mut self, value: &Value, mapping_context: bool, simple_key: bool) {
        match value {
            ...
            Value::Object(map) => {
                self.root = false;
                self.emit_block_mapping(map);
            }
            Value::Array(items) => {
                self.root = false;
                self.emit_block_sequence(items, mapping_context);
            }
            scalar => {
