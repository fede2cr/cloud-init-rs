...
/// `util.uptime`: the first field of `/proc/uptime`, kept as the string upstream
/// interpolates into the welcome message and parses for `status.json`.
///
/// Upstream falls back to `time.monotonic()` when `/proc/uptime` is missing,
/// which cannot happen on the Linux systems this port targets.
...
/// `datetime.now(timezone.utc).isoformat()`, which differs from `str()` on the
/// same value only in the separator.
pub fn format_python_isoformat_utc(epoch: f64) -> String {
    format_python_datetime_utc(epoch).replacen(' ', "T", 1)
}
