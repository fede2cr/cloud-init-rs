...
///
/// `CLOUD_CFG` is deliberately absent: upstream applies it as a higher-priority
/// layer in [`crate::merger`], not as a replacement for `/etc/cloud/cloud.cfg`.
pub fn fetch_base_config(
    ...
    let sources = vec![
        builtin::cfg_builtin(),
        read_conf_with_confd(builtin::CLOUD_CONFIG, instance_data, limits)?,
        read_conf(run_dir.as_ref().join("cloud.cfg"), instance_data, limits)?,
