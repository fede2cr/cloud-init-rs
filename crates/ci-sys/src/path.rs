...
/// `util.ensure_dir`: create `path` and its parents, then set `mode`.
///
/// The mode is applied only when the directory had to be created, which is what
/// keeps a hand-tightened state directory from being loosened on every boot.
pub fn ensure_dir(path: impl AsRef<Path>, mode: u32) -> io::Result<()> {
    let path = path.as_ref();
    if path.is_dir() {
        return Ok(());
    }
...
/// `util.sym_link`: `force` replaces an existing link rather than failing.
pub fn sym_link(
    ...
    if force && link.symlink_metadata().is_ok() {
