...
impl Message {
    ...
    /// `MIMEMultipart()`, the container `UserDataProcessor.process` fills.
    pub fn multipart(boundary: &str) -> Self {
        Self {
            ...
            body: Body::Multipart(Vec::new()),
        }
    }

    /// `MIMENonMultipart(maintype, subtype)`, whose two headers arrive in this
    /// order and are then shuffled by whatever replaces them.
    pub fn non_multipart(content_type: &str, payload: Vec<u8>) -> Self {
        Self {
            headers: vec![
    ...
    /// Value for exactly `name`, last one wins.
    ///
    /// Handlers are handed `dict(part)`, which is a plain dictionary keyed by
    /// the header names as written, so any lookup a handler does is
    /// case-sensitive and sees only the last of a repeated header. See
    /// docs/COMPAT.md.
    ...
    /// `_replace_header`: delete every occurrence, then append, so the header
    ...
    /// `add_header`: appends without removing anything, so a name can repeat.
    ...
    /// The `boundary` parameter of this message's `Content-Type`.
    ...
    /// `str(msg)`: `Generator` with `maxheaderlen=0`, so nothing is folded and
    /// the body is written exactly as it is held.
    #[must_use]
    ...
    fn write_to(&self, out: &mut Vec<u8>) {
        ...
        out.push(b'\n');
        match &self.body {
            Body::Leaf(raw) => out.extend_from_slice(raw),
            Body::Multipart(parts) => {
                let boundary = self.boundary().unwrap_or_default();
                // The opening delimiter is written even when there is nothing
                // to delimit, so an empty multipart still shows both markers.
