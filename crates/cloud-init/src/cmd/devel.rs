...
//!
//! `hotplug-hook` depends on subsystems that land in a later phase.
...
pub enum DevelCommand {
    ...
    #[command(name = "net-convert")]
    NetConvert(net_convert::Args),
    /// Audit a system against the cloud-init-rs filesystem contract.
    ///
    /// Not an upstream subcommand: it exists because the two implementations
    /// share their state directories.
    #[command(name = "verify-layout")]
    VerifyLayout(verify_layout::Args),
}
...
pub fn run(args: &Args) -> u8 {
    match &args.command {
        ...
        DevelCommand::NetConvert(convert) => net_convert::run(convert),
        DevelCommand::VerifyLayout(layout) => verify_layout::run(layout),
    }
}
